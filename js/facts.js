// AI-Powered Fun Facts Fetcher using OpenAI + SerpAPI
const FactsFetcher = {
    cache: new Map(),
    requestQueue: [],
    isProcessing: false,
    enabled: true, // Enabled when using backend server to avoid CORS issues
    ...
    /**
     * Fetch an AI-generated fact using OpenAI GPT with SerpAPI search
    ...
    async fetchAIFact(name, type) {
        try {
            ...
            const fact = await this.generateFactWithOpenAI(name, type, searchResults);
     ...
     */
    async searchWithSerpAPI(query, type) {
        try {
            const searchQuery = `${query} ${type} interesting facts`;
            ...
            if (CONFIG.USE_BACKEND) {
                // Use backend proxy
                const backendUrl = `${CONFIG.BACKEND_URL}/serp?q=${encodeURIComponent(searchQuery)}&num=3`;
                response = await fetch(backendUrl, { signal: controller.signal });
            } else {
                // Direct API call (CORS will block in browsers without backend proxy)
                const apiUrl = `${CONFIG.SERPAPI_SEARCH}?q=${encodeURIComponent(searchQuery)}&api_key=${CONFIG.SERPAPI_KEY}&num=3`;
            ...
            if (data.answer_box) {
                searchSummary += data.answer_box.answer || data.answer_box.snippet || '';
            }
            
            if (data.organic_results && data.organic_results.length > 0) {
                const topResults = data.organic_results.slice(0, 3);
                searchSummary += ' ' + topResults.map(r => r.snippet).join(' ');
            }
            
            if (data.knowledge_graph) {
                searchSummary += ' ' + (data.knowledge_graph.description || '');
            }
            
            return searchSummary.trim() || 'No specific information found';
            
        } catch (error) {
            console.warn('SerpAPI search failed:', error.message);
            return 'General information available';
        }
    },
